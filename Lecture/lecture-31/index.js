...
console.log("Shawqi\tHatem"); // \t -> عبارة عن تاب
...
email = email.trim(); // بتلغي المسافات
...
console.log(isSubscribed.includes(true)); // true
...
// Task - 01
// write a function `upperFirstCharacter(string)` that returns the `string` with the
...
// Task - 02
// write a function `truncate(string, maxLength)` that checks the length of the
// `string` and, if it exceeds `maxLength` replace the end of the `string` with the
...
// Task - 03
// write a function `extractAmountValue` that would extract the numeric value from
// such string and return it.
// extractAmountValue("$120") -> 120.

// 0. all amounts will be prefixed by dollar sign.
// 1. all amounts will be integers and consisted of 3 digits.
// 2. we might have more than one amount in the string.

console.log("--------------- Task 3 -------------------");
//String طلع الارقام من ال Function هان بدنا نعمل
